...
from notification_commons.config import SMS_NOTIFICATION_TOPIC, SMS_TIMEOUT_MS
from kafka_service.kafka_consumer import Consumer

class SmsMessage(ProviderInterface):
    def __init__(self):
    ...
    def send_notification(self):
        ...
        consumer_obj.set_consumer(SMS_NOTIFICATION_TOPIC)
        consumer = consumer_obj.get_consumer()

        redis = RedisHandler()
        while True:
            msg_pack = consumer.poll(timeout_ms=SMS_TIMEOUT_MS)
